...
interface AccountingDepartmentProps {
  subDepartmentId: string | null;
}

const AccountingDepartment: React.FC<AccountingDepartmentProps> = ({ subDepartmentId }) => {
  const [showGoalPlan, setShowGoalPlan] = useState(false);
  const departmentName = "Bộ phận kế toán";
  
  // Find the current sub-department name if applicable
  let subDepartmentName = "";
  if (subDepartmentId === 'accounting-admin') subDepartmentName = "Phòng KT Hành chính";
  if (subDepartmentId === 'accounting-tax') subDepartmentName = "Phòng KT thuế";
      ...
      {!subDepartmentId ? (
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4">
            <div className="bg-white p-3 md:p-4 rounded-lg shadow border border-gray-200">
              <h3 className="font-medium text-base md:text-lg mb-2">Phòng KT Hành chính</h3>
              <p className="text-sm text-gray-600 mb-2 md:mb-3">Quản lý kế toán hành chính</p>
            ...
            <div className="bg-white p-3 md:p-4 rounded-lg shadow border border-gray-200">
              <h3 className="font-medium text-base md:text-lg mb-2">Phòng KT thuế</h3>
              <p className="text-sm text-gray-600 mb-2 md:mb-3">Quản lý kế toán thuế</p>
              <div className="text-right">
                <button className="text-blue-600 hover:text-blue-800 text-sm font-medium">
                  Xem chi tiết
                </button>
