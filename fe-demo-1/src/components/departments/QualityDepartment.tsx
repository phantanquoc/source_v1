...
import { Users, FileText, Settings, FileCheck } from 'lucide-react';
import GoalPlanView from '../GoalPlanView';
import QuyTrinhView from '../QuyTrinhView';

interface QualityDepartmentProps {
  subDepartmentId: string | null;
  setActiveSubDepartment?: (id: string | null) => void;
}

const QualityDepartment: React.FC<QualityDepartmentProps> = ({ subDepartmentId, setActiveSubDepartment }) => {
  const [showGoalPlan, setShowGoalPlan] = useState(false);
  const departmentName = "Bộ phận chất lượng";

  // Find the current sub-department name if applicable
  let subDepartmentName = "";
  if (subDepartmentId === 'quality-personnel') subDepartmentName = "Thông tin nhân sự";
  if (subDepartmentId === 'quality-office') subDepartmentName = "Chất lượng khối văn phòng";
  if (subDepartmentId === 'quality-production') subDepartmentName = "Chất lượng khối sản xuất";
  if (subDepartmentId === 'quality-process') subDepartmentName = "Quy trình";
  ...
  return (
    <div>
      ...
      {!subDepartmentId ? (
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4">
            <div className="bg-white p-3 md:p-4 rounded-lg shadow border border-gray-200">
              <h3 className="font-medium text-base md:text-lg mb-2">Thông tin nhân sự</h3>
              <p className="text-sm text-gray-600 mb-2 md:mb-3">Quản lý thông tin và hoạt động của phòng nhân sự</p>
            ...
            <div className="bg-white p-3 md:p-4 rounded-lg shadow border border-gray-200">
              <h3 className="font-medium text-base md:text-lg mb-2">Chất lượng khối văn phòng</h3>
              <p className="text-sm text-gray-600 mb-2 md:mb-3">Quản lý chất lượng các hoạt động văn phòng</p>
              <div className="text-right">
                <button className="text-blue-600 hover:text-blue-800 text-sm font-medium">
                  Xem chi tiết
                </button>
            ...
            <div className="bg-white p-3 md:p-4 rounded-lg shadow border border-gray-200">
              <h3 className="font-medium text-base md:text-lg mb-2">Chất lượng khối sản xuất</h3>
              <p className="text-sm text-gray-600 mb-2 md:mb-3">Quản lý chất lượng các hoạt động sản xuất</p>
              <div className="text-right">
                <button className="text-blue-600 hover:text-blue-800 text-sm font-medium">
                  Xem chi tiết
                </button>
            ...
            <div className="bg-white p-3 md:p-4 rounded-lg shadow border border-gray-200">
              <h3 className="font-medium text-base md:text-lg mb-2">Quy trình</h3>
              <p className="text-sm text-gray-600 mb-2 md:mb-3">Quản lý các quy trình chất lượng</p>
              <div className="text-right">
                <button
                  className="text-blue-600 hover:text-blue-800 text-sm font-medium"
                  onClick={() => {
                    if (setActiveSubDepartment) {
                      setActiveSubDepartment('quality-process');
                    }
                  }}
                >
                  Xem chi tiết
      ...
      ) : subDepartmentId === 'quality-process' ? (
        <QuyTrinhView onBack={() => {
          if (setActiveSubDepartment) {
