...
interface ProductionDepartmentProps {
  subDepartmentId: string | null;
}

const ProductionDepartment: React.FC<ProductionDepartmentProps> = ({ subDepartmentId }) => {
  const [showGoalPlan, setShowGoalPlan] = useState(false);
  const departmentName = "Bộ phận sản xuất";
  
  // Find the current sub-department name if applicable
  let subDepartmentName = "";
  if (subDepartmentId === 'production-management') subDepartmentName = "Phòng QLSX";
  if (subDepartmentId === 'production-warehouse') subDepartmentName = "Quản lý kho";
      ...
      {!subDepartmentId ? (
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4">
            <div className="bg-white p-3 md:p-4 rounded-lg shadow border border-gray-200">
              <h3 className="font-medium text-base md:text-lg mb-2">Phòng QLSX</h3>
              <p className="text-sm text-gray-600 mb-2 md:mb-3">Quản lý sản xuất</p>
            ...
            <div className="bg-white p-3 md:p-4 rounded-lg shadow border border-gray-200">
              <h3 className="font-medium text-base md:text-lg mb-2">Quản lý kho</h3>
              <p className="text-sm text-gray-600 mb-2 md:mb-3">Quản lý kho hàng</p>
              <div className="text-right">
                <button className="text-blue-600 hover:text-blue-800 text-sm font-medium">
                  Xem chi tiết
                </button>
