...
interface BusinessDepartmentProps {
  subDepartmentId: string | null;
}

const BusinessDepartment: React.FC<BusinessDepartmentProps> = ({ subDepartmentId }) => {
  const [showGoalPlan, setShowGoalPlan] = useState(false);
  const departmentName = "Bộ phận kinh doanh";
  
  // Find the current sub-department name if applicable
  let subDepartmentName = "";
  if (subDepartmentId === 'business-international') subDepartmentName = "Phòng KD Quốc Tế";
  if (subDepartmentId === 'business-domestic') subDepartmentName = "Phòng KD Nội Địa";
      ...
      {!subDepartmentId ? (
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4">
            <div className="bg-white p-3 md:p-4 rounded-lg shadow border border-gray-200">
              <h3 className="font-medium text-base md:text-lg mb-2">Phòng KD Quốc Tế</h3>
              <p className="text-sm text-gray-600 mb-2 md:mb-3">Quản lý hoạt động kinh doanh quốc tế</p>
            ...
            <div className="bg-white p-3 md:p-4 rounded-lg shadow border border-gray-200">
              <h3 className="font-medium text-base md:text-lg mb-2">Phòng KD Nội Địa</h3>
              <p className="text-sm text-gray-600 mb-2 md:mb-3">Quản lý hoạt động kinh doanh trong nước</p>
              <div className="text-right">
                <button className="text-blue-600 hover:text-blue-800 text-sm font-medium">
                  Xem chi tiết
                </button>
