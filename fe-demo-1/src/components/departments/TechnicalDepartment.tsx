...
interface TechnicalDepartmentProps {
  subDepartmentId: string | null;
}

const TechnicalDepartment: React.FC<TechnicalDepartmentProps> = ({ subDepartmentId }) => {
  const [showGoalPlan, setShowGoalPlan] = useState(false);
  const departmentName = "Bộ phận kỹ thuật";
  
  // Find the current sub-department name if applicable
  let subDepartmentName = "";
  if (subDepartmentId === 'technical-system') subDepartmentName = "Phòng QLHTM";
  if (subDepartmentId === 'technical-mechanical') subDepartmentName = "Phòng cơ- điện";
      ...
      {!subDepartmentId ? (
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4">
            <div className="bg-white p-3 md:p-4 rounded-lg shadow border border-gray-200">
              <h3 className="font-medium text-base md:text-lg mb-2">Phòng QLHTM</h3>
              <p className="text-sm text-gray-600 mb-2 md:mb-3">Quản lý hệ thống máy</p>
            ...
            <div className="bg-white p-3 md:p-4 rounded-lg shadow border border-gray-200">
              <h3 className="font-medium text-base md:text-lg mb-2">Phòng cơ- điện</h3>
              <p className="text-sm text-gray-600 mb-2 md:mb-3">Quản lý hệ thống cơ điện</p>
              <div className="text-right">
                <button className="text-blue-600 hover:text-blue-800 text-sm font-medium">
                  Xem chi tiết
                </button>
